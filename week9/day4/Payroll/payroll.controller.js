...
const { handlePayrollTransaction } = require('./payrollService');
...
exports.updatePayroll = async (req, res) => {
  let { Salary, bonus, deductions } = req.body;
  ...
  try {
    const beforePayroll = await Payroll.findById(req.params.id).lean();
    if (!beforePayroll) return res.status(404).json({ message: 'Payroll not found' });
    req.before = beforePayroll;
    const payroll = await Payroll.findByIdAndUpdate(
      req.params.id,
      { Salary, bonus, deductions, grossPay, netPay },
      { new: true }
    );
...
exports.getAllPayrolls = async (req, res) => {
  try {
    const payrolls = await Payroll.find().populate('employeeId', 'first_name last_name');
    res.status(200).json(payrolls);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
